...
func _process(delta):
	...
	g =  stepify(255-(T0[1]-1.1)*100,0.01)  #print(stepify(255-T0[1]*100 ,0.01))
	b =  stepify(255-(T0[1]-1.1)*100,0.01)
	$Tween.interpolate_property(Bkgnd,"modulate",Bkgnd.modulate,Color8(255,g,b,225),0.2,Tween.TRANS_LINEAR)
	$Tween.start()
	
	#Rishika
	TYear = TYear + delta
	var Scale = get_node("../CanvasLayer/Sliders/TotalSlider").value/70.0
	CO2emissions = CO2emissions0*Scale
	CH4emissions = CH4emissions0*Scale
	N2Oemissions = N2Oemissions0*Scale
	var CO2equivemissions = CO2emissions + 32*CH4emissions + 265*N2Oemissions
	get_node("../CanvasLayer/Sliders/TotalSlider").hint_tooltip = str("%3.1f" % CO2equivemissions) + " Gton/yr CO2-eq"
	...
	get_node("../CanvasLayer/Sliders/HBoxContainer/AerTLabel").set_text("  Short-lived pollution:  \n        " + str("%3.2f" % stepify(fa[1],0.01)) + " W/m2 (cooling)")
